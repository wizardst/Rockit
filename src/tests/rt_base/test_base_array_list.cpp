...
RT_RET unit_test_array_list(INT32 index, INT32 total_index) {
    ...
    for (INT32 i = 0; i < 1000; i++) {
        INT32_array[i] = i;
        array_list_add(rt_list, &INT32_array[i]);
    }
    ...
    */
    ...
    array_list_set(rt_list, 234, &INT32_array[234]);
    array_list_set(rt_list, 235, &INT32_array[235]);
    ...
    */
    ...
    CHECK_EQ(array_list_contains(rt_list, &INT32_array[100]), RT_TRUE);
    CHECK_EQ(array_list_contains(rt_list, &INT32_array[1001]), RT_FALSE);

    array_list_remove(rt_list, &INT32_array[236]);
    array_list_remove(rt_list, &INT32_array[237]);
